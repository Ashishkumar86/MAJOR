            ...
            <div className="max-w-6xl mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-1/2">
                        <FaArrowLeft className="text-black w-[22px] h-[22px] cursor-pointer mb-3" onClick={()=>navigate("/")}/>
                        <img
                    ...
                    {/* Course info */}
                    <div className="flex-1 space-y-2 mt-[20px] text-left">
                        <h2 className="text-2xl font-bold">{selectedCourse?.title}</h2>
                        <p className="text-gray-600">{selectedCourse?.subTitle}</p>

                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                                <span className="flex items-center gap-1 text-yellow-500 font-medium"><MdStar/>{avgRating}</span>
                                <span className="text-gray-400">(1,200 Reviews)</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-lg font-semibold text-black">Rs {selectedCourse?.price}</span>
                                <span className="line-through text-sm text-gray-400">Rs 599</span>
                            </div>
                            ...
                            {!isEnrolled
                                ? <button
                                    className="bg-[#5A4BDA] text-white px-4 py-2 w-fit rounded hover:bg-gray-700 mt-3 cursor-pointer"
                                    onClick={()=> userData?._id && handleEnroll(userData._id, courseId)}
                                  >Enroll Now</button>
                                : <button
                                    className="bg-green-100 text-green-500 px-6 py-2 rounded hover:bg-gray-700 mt-3 w-fit cursor-pointer"
                ...
                <div>
                    ...
                    <ul className="list-disc pl-6 text-gray-700 space-y-0 text-sm">
                ...
                <div>
                    ...
                    <p className="text-gray-700 text-sm">Beginners, Aspiring Developers and Professionals looking to Upgrade Skills.</p>
                </div>
                ...
                <div className="mt-8 border-t pt-6">
                    ...
                    <div className="mb-4">
                        <div className="flex gap-1 mb-2">
                            {[1,2,3,4,5].map((star)=>(
                                <MdStar key={star} onClick={()=>setRating(star)} className={star <= rating ? "fill-amber-300" : "fill-gray-300"} />
                            ))}
                        </div>
                        <textarea
                        ...
                        />
                        <button className="bg-[#5A4BDA] text-white mt-3 px-4 py-2 rounded hover:bg-gray-800" disabled={loading} onClick={handleReview}>
