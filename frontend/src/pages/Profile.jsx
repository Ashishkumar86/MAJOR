      ...
      <div className="bg-white shadow-lg rounded-2xl p-8 max-w-xl w-full relative">
        ...
        <div className="mt-6 space-y-4 ">
          <div className="text-sm">
            <span className="font-semibold text-gray-700">Email : {userData.email}</span>
          </div>
          <div className="text-sm">
            <span className="font-semibold text-gray-700">Bio : {userData.description}</span>
          </div>
          <div className="text-sm">
            <span className="font-semibold text-gray-700">Enrolled Courses : {userData.enrolledCourses.length}</span>
          </div>
        </div>
        <div className="mt-6 flex justify-center gap-4">
          <button className="px-4 py-2 bg-[#5A4BDA] text-white rounded-2xl active:bg-[#4b4b4b] cursor-pointer transition" onClick={() => navigate("/editprofile")}>Edit Profile</button>
        </div>
      </div>
    </div>
  );
