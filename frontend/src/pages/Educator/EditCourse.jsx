  ...
  const handlePublishToggle = async () => {
    ...
    try {
      ...
      if(updateData.isPublished){
        ...
        if(!courseData.some(c => c._id === courseId)) updateCourses.push(updateData)
    ...
    <div className="max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-center gap-[20px] md:justify-between flex-col md:flex-row mb-6 relative">
        ...
        <div className="space-x-2 space-y-2">
          <button className="bg-[#5A4BDA] text-white px-4 py-2 rounded-md" onClick={()=>navigate(`/createlecture/${selectedCourse?._id}`)}>Go to Lecture Page</button>
        </div>
      </div>
      ...
      <div className="bg-gray-50 p-6 rounded-md">
        ...
        <div className="flex gap-3 mb-6">
          {!isPublished
            ? <button className="bg-green-100 text-green-600 px-4 py-2 rounded-md border" onClick={handlePublishToggle}>
                {loading ? <ClipLoader size={16} color="green"/> : "Click to Publish"}
              </button>
            : <button className="bg-red-100 text-red-600 px-4 py-2 rounded-md border" onClick={handlePublishToggle}>
                {loading ? <ClipLoader size={16} color="red"/> : "Click to Unpublish"}
              </button>
          }
          <button className="bg-red-600 text-white px-4 py-2 rounded-md" onClick={handleRemoveCourse}>
            {loading1 ? <ClipLoader size={16} color="white"/> : "Remove Course"}
          </button>
        </div>
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            ...
            <textarea id="des" className="w-full border h-24 px-4 py-2 rounded-md" placeholder="Add text .." onChange={(e) => setDescription(e.target.value)} value={description}></textarea>
          </div>

          <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0">
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 mb-1">Course Category</label>
              <select id="category" className="w-full border rounded-md px-4 py-2 bg-white" onChange={(e) => setCategory(e.target.value)} value={category}>
            ...
            <div className="flex-1">
              ...
              <select id="level" className="w-full border rounded-md px-4 py-2 bg-white" onChange={(e) => setLevel(e.target.value)} value={level}>
            ...
            <div className="flex-1">
              ...
              <input type="number" id="price" className="w-full border px-4 py-2 rounded-md" placeholder="Rs" onChange={(e) => setPrice(e.target.value)} value={price}/>
            </div>
          </div>
          ...
          <div className="relative w-[300px]">
            <img src={frontendImage} className="w-full h-full border rounded-md object-cover cursor-pointer" onClick={() => thumb.current.click()}/>
            <FaRegEdit className="w-[20px] h-[20px] absolute top-2 right-2 cursor-pointer" onClick={() => thumb.current.click()}/>
          </div>

          <div className="flex items-center justify-start gap-[15px]">
            <button className="bg-[#e9e8e8] hover:bg-red-200 text-black border cursor-pointer px-4 py-2 rounded-md" onClick={() => navigate("/courses")}>Cancel</button>
            <button className="bg-[#5A4BDA] text-white px-7 py-2 rounded-md hover:bg-gray-500 cursor-pointer" onClick={handleEditCourse}>
