        ...
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4"> 
            <div className="bg-white shadow-xl rounded-xl w-full max-w-2xl p-6">
                ...
                <div className="flex gap-4 mb-6">
                    <button className="flex items-center gap-2 px-4 rounded-md bg-gray-200 hover:bg-gray-300" onClick={()=>navigate(`/editcourse/${courseId}`)} ><FaArrowLeft/>Back to Course</button>
                    <button className="px-5 py-2 rounded-md bg-[#5A4BDA] text-white hover:bg-gray-600 transition-all text-sm font-medium shadow" disabled={loading} onClick={handleCreateLecture}>{loading ? <ClipLoader size={30} color="white"/>:"+ Create Lecture"}</button>
                </div>
                {/* lecture list*/}
                ...
                <div className="space-y-2">
                    {lectureData?.map((lecture, index)=>(
                        <div key={index} className="bg-gray-100 rounded-md flex justify-between items-center p-3 text-sm font-medium text-gray-700">
                            ...
                            <span>
                                <FaRegEdit className="text-gray-700 hover:text-gray-700 cursour-pointer" onClick={()=>navigate(`/editlecture/${courseId}/${lecture._id}`)}/>
                            </span>
                        </div>
