...
function AllCourses(){
    ...
    return(
        ...
        <div className="flex min-h-screen bg-gray-50">
            ...
            <button className="fixed top-20 left-4 bg-black text-white px-3 py-1 rounded md:hidden border-2 border-white z-50" onClick={()=>setIsSidebarVisible(prev=>!prev)}>
                {isSidebarVisible ? 'hide' : 'show'} Filters
            </button>

            {/* sidebar*/}
            <aside className={`w-[260px] h-screen overflow-y-0 bg-[#5A4BDA] fixed top-0 left-0 p-6 py-[130px] border-r border-gray-200 shadow-md transition-transform duration-300 z-40 ${isSidebarVisible ? "translate-x-0" : "-translate-x-full"} md:block md:translate-x-0`}>
                <h2 className="text-xl font-bold flex items-center justify-center gap-2 text-gray-50 mb-6"><FaArrowLeft className="text-white" onClick={()=>navigate("/home")}/>Filter by Category</h2>
                ...
                <form action="" onSubmit={(e)=>e.preventDefault()} className="space-y-4 text-sm bg-gray-600 border-white text-[white] border p-[20px] rounded-2xl">
                    <button className="px-[10px] py-[10px] bg-black text-white rounded-[10px] text-[15px] font-light flex cursor-pointer " onClick={()=>navigate("/search")}>Search with AI <img src={AI} className="w-[30px] h-[30px] rounded-full"/></button>
            ...
            <main className="w-full transition-all duration-300 py-[130px] md:pl-[320px] flex items-start justify-start content-start flex-wrap gap-8 px-[20px]">
                {
