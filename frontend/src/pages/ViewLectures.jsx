    ...
    useEffect(()=>{
        const fetchLectures = async () => {
            try {
                ...
                const fetchedLectures = result.data?.lectures || []
                setLectures(fetchedLectures)
                setSelectedLecture(fetchedLectures[0] || null)  
            } catch (error) {
                    ...
                    {lectures.length > 0    
                        ? (lectures.map((lecture, index) => (
                            <button 
                                ...
                                className={`flex items-center justify-between p-3 rounded-lg border transition text-left ${selectedLecture?._id === lecture._id ? "bg-gray-200 border-gray-500" : "hover:bg-gray-50 border-gray-300"}`}
                            >
                ...
                {creatorData && 
                    <div className='mt-4 border-t pt-4'>
                        ...
                        <div className='flex items-center gap-4'>
                            <img src={creatorData.photoUrl} className='w-14 h-14 rounded-full object-cover'/>
                            <div>
